        ...
        <div className="grid grid-cols-1 gap-8 mb-24">
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-2xl border border-slate-100 dark:border-slate-800 hover-shadow transition-all duration-500">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-6">
              <div className="flex flex-wrap items-center gap-2 p-1 md:p-1.5 bg-slate-50 dark:bg-slate-800 rounded-2xl md:rounded-[1.5rem] shadow-inner">
                {TONES.map((t) => (
                  <button
                    key={t.id}
                    onClick={() => setTone(t.id)}
                    className={`flex items-center gap-2 px-3 md:px-6 py-2 md:py-3 rounded-[1rem] md:rounded-[1.2rem] font-black text-[10px] md:text-xs uppercase tracking-widest transition-all duration-300 ${
                      tone === t.id 
                        ? "bg-white dark:bg-slate-700 text-violet-600 shadow-lg scale-105" 
                        : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"
                    }`}
                  >
                    <t.icon className="w-3.5 h-3.5 md:w-4 h-4" />
                    <span className="inline">{t.label}</span>
                  </button>
                ))}
              </div>

              <div className="flex flex-wrap items-center gap-4 md:gap-6 text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest md:tracking-[0.2em]">
                {status === "processing" ? (
                  <div className="flex items-center gap-2 text-violet-500 animate-pulse">
                    <RefreshCw className="w-3.5 h-3.5 md:w-4 h-4 animate-spin" />
                    <span>Processing...</span>
                  </div>
                ) : (
                  <div className="flex items-center gap-2 text-emerald-500 whitespace-nowrap">
                    <CheckCircle2 className="w-3.5 h-3.5 md:w-4 h-4" />
                    <span>Syncing Active</span>
                  </div>
                )}
                <div className="hidden xs:block w-px h-4 bg-slate-200 dark:bg-slate-800" />
                <div className="flex items-center gap-2 whitespace-nowrap">
                  <Clock className="w-3.5 h-3.5 md:w-4 h-4 text-violet-500/50" />
                  <span>{metrics.readingTime}m read</span>
                </div>
                <div className="flex items-center gap-2 whitespace-nowrap">
                  <FileText className="w-3.5 h-3.5 md:w-4 h-4 text-emerald-500/50" />
            ...
            <div className="relative group">
              ...
              <textarea
                ...
                className="w-full h-[350px] p-10 bg-slate-50/50 dark:bg-slate-800/80 rounded-[2rem] border-2 border-slate-100 dark:border-slate-700 focus:outline-none focus:border-violet-500/50 transition-all font-medium text-lg text-slate-700 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-600 resize-none leading-relaxed"
              />
              ...
              >
                ...
                <div className="flex items-center justify-center gap-4">
                  <button 
                    ...
                    className={`px-8 py-3 rounded-full font-black text-[10px] uppercase tracking-widest transition-all ${viewMode === "result" ? "bg-slate-900 text-white dark:bg-white dark:text-slate-900 shadow-lg" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"}`}
                  >
                    Result View
                  </button>
                  <button 
                    onClick={() => setViewMode("compare")}
                    className={`px-8 py-3 rounded-full font-black text-[10px] uppercase tracking-widest transition-all ${viewMode === "compare" ? "bg-slate-900 text-white dark:bg-white dark:text-slate-900 shadow-lg" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"}`}
                  >
                    Comparison Mode
                  </button>
                </div>
                    ...
                    {viewMode === "result" ? (
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between px-4">
                            ...
                            <div className="flex items-center gap-2">
                              <button onClick={copyResult} className="p-2 md:p-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl text-slate-500 hover:text-violet-600 transition-all border border-slate-200 dark:border-slate-700 active:scale-90" title="Copy">
                                <Copy className="w-4 h-4 md:w-4.5 md:h-4.5" />
                              </button>
                              <button onClick={downloadResult} className="p-2 md:p-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl text-slate-500 hover:text-violet-600 transition-all border border-slate-200 dark:border-slate-700 active:scale-90" title="Download">
                                <Download className="w-4 h-4 md:w-4.5 md:h-4.5" />
                              </button>
                              <button onClick={() => setTrigger(t => t + 1)} className="p-2 md:p-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl text-slate-500 hover:text-violet-600 transition-all border border-slate-200 dark:border-slate-700 active:scale-90" title="Regenerate">
                                <RefreshCw className="w-4 h-4 md:w-4.5 md:h-4.5" />
                              </button>
                            </div>
                          </div>
                          <textarea
                            readOnly
                            value={output}
                            className="w-full h-[400px] p-8 md:p-10 bg-emerald-50/40 dark:bg-emerald-950/40 rounded-[2rem] border-2 border-emerald-200 dark:border-emerald-800/50 focus:outline-none font-medium text-lg text-slate-900 dark:text-slate-100 resize-none leading-relaxed shadow-inner"
                          />
                        </div>
                      </motion.div>
                    ) : (
                      ...
                      >
                        <div className="space-y-4">
                           ...
                           <div className="p-8 bg-slate-100/50 dark:bg-slate-800/80 rounded-[2rem] border border-slate-200 dark:border-slate-700 text-sm leading-relaxed text-slate-600 dark:text-slate-400 line-through decoration-red-500/40">
                             {input}
                           </div>
                        </div>
                        <div className="space-y-4">
